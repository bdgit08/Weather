...
class InputActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        container.setBackgroundColor(Color.parseColor(FormatTimes.timeBackground()))
    ...
    override fun onClick(p0: View?) {
        if (enabledInput()) {
            ...
            intent.putExtra(WeatherInfoActivity.CITY, editTextKota.text.toString())
            startActivity(intent)
        }
    }

    private fun enabledInput(): Boolean {
        if (editTextName.length() > 0 &&editTextKota.length() > 0 && editTextKodePos.length() > 4) {
            return true
        } else {
            if (editTextName.length() == 0) {
                editTextName.error = "Nama tidak boleh kosong"
            }else if(editTextKota.length()==0){
                editTextKota.error = "Kota tidak boleh kosong"
